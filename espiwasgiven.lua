...
local function createLine(thickness, color, zindex)
...
local function createText(size, font, color, center, outline)
...
local function createCircle(filled, thickness, color, zindex)
...
local function createSquare(filled, thickness, color, zindex)
...
local function createDrawings()
    local d = {
        ...
        Tracer = createLine(1, Color3.new(), 2),
        TracerOutline = createLine(3, Color3.new(), 1),
        HeadDot = createCircle(true, 1, Color3.new(), 2),
        HeadDotOutline = createCircle(false, 3, Color3.new(), 1),
        HealthBG = createSquare(true, 1, Color3.new(), 1),
        HealthFill = createSquare(true, 1, Color3.new(), 2),
        Name = createText(14, 2, Color3.new(), true, true),
        Distance = createText(14, 2, Color3.new(), true, true),
        HealthT = createText(14, 2, Color3.new(), false, true),
        Highlight = Instance.new("Highlight")
    }
    
    for i = 1, 8 do
        d.Corners[i] = createLine(1, Color3.new(), 2)
        d.CornerOutlines[i] = createLine(3, Color3.new(), 1)
    end
    for i = 1, 4 do
        d.Box[i] = createLine(1, Color3.new(), 2)
        d.BoxOutlines[i] = createLine(3, Color3.new(), 1)
    end
    for i = 1, 15 do
        d.Skeleton[i] = createLine(1, Color3.new(), 2)
        d.SkeletonOutlines[i] = createLine(3, Color3.new(), 1)
    end
    return d
end
...
local function removeDrawings(d)
    ...
    d.Tracer:Remove()
    d.TracerOutline:Remove()
    d.HeadDot:Remove()
    d.HeadDotOutline:Remove()
    d.HealthBG:Remove()
    d.HealthFill:Remove()
    d.Name:Remove()
    d.Distance:Remove()
...
local function setLineParams(line, outline, from, to, t, c, oc, v, ov)
    if line then
        ...
        line.Thickness = t; line.Color = c
        line.Visible = v
    end
    if outline then
        outline.From = from; outline.To = to
        outline.Thickness = t + 2; outline.Color = oc
        outline.Visible = ov
    end
end
...
function ESP:Remove(model)
    if self.Tracked[model] then
        removeDrawings(self.Tracked[model])
        self.Tracked[model] = nil
    end
end

function ESP:Clear()
    for model, _ in pairs(self.Tracked) do
        self:Remove(model)
    end
    ...
    for model, d in pairs(self.Tracked) do
        ...
        if S.Enabled and root and model.Parent then
            ...
            if onScreen and dist <= S.MaxDistance then
                ...
                if S.Skeletons then
                    local rigType = model:FindFirstChild("UpperTorso") and "R15" or "R6"
                    local joints = skelData[rigType]
                        ...
                        if pair then
                            ...
                            if p1 and p2 then
                                ...
                                if vis1 or vis2 then
                                    setLineParams(d.Skeleton[i], d.SkeletonOutlines[i], Vector2.new(pos1.X, pos1.Y), Vector2.new(pos2.X, pos2.Y), S.Thickness, S.SkeletonColor, S.OutlineColor, true, S.BoxOutlines)
                                else
                                    d.Skeleton[i].Visible = false; d.SkeletonOutlines[i].Visible = false
                                end
                            else
                                d.Skeleton[i].Visible = false; d.SkeletonOutlines[i].Visible = false
                            end
                ...
                if S.Names then
                    ...
                    d.Name.Color = S.Color
                    d.Name.Size = S.TextSize
                    d.Name.Font = S.Font
                    d.Name.Position = Vector2.new(x + w / 2, y - d.Name.TextBounds.Y - 2)
                    d.Name.Visible = true
                else
                    d.Name.Visible = false
                end
                ...
                if S.Distances then
                    ...
                    d.Distance.Color = S.Color
                    d.Distance.Size = S.TextSize
                    d.Distance.Font = S.Font
                ...
                if S.HealthBar and hum then
                    ...
                    d.HealthBG.Color = S.OutlineColor
                    d.HealthBG.Visible = S.BoxOutlines
                    
                    d.HealthFill.Size = Vector2.new(1, barH)
                    d.HealthFill.Position = Vector2.new(x - 4, y + (h - barH))
                    d.HealthFill.Color = S.HealthBarColor
                    d.HealthFill.Visible = true
                    
                    if S.HealthText then
                        d.HealthT.Text = tostring(math.floor(hum.Health))
                        d.HealthT.Color = S.HealthBarColor
                        d.HealthT.Size = S.TextSize - 2
                        d.HealthT.Font = S.Font
                        d.HealthT.Position = Vector2.new(x - 5 - d.HealthT.TextBounds.X - 2, y + (h - barH) - 5)
                        d.HealthT.Visible = true
                    else
                        d.HealthT.Visible = false
                    end
                else
                ...
                if S.HeadDots and head then
                    local hPos = camera:WorldToViewportPoint(head.Position)
                    d.HeadDot.Position = Vector2.new(hPos.X, hPos.Y)
                    d.HeadDot.Radius = S.HeadDotSize
                    d.HeadDot.Color = S.Color
                    d.HeadDot.Visible = true
                    
                    d.HeadDotOutline.Position = Vector2.new(hPos.X, hPos.Y)
                    d.HeadDotOutline.Radius = S.HeadDotSize
                    d.HeadDotOutline.Color = S.OutlineColor
                    d.HeadDotOutline.Visible = S.BoxOutlines
                else
                    d.HeadDot.Visible = false
                    d.HeadDotOutline.Visible = false
                end
                ...
                if S.Chams then
                    d.Highlight.Parent = model
                    d.Highlight.FillColor = S.ChamsFillColor
                    d.Highlight.FillTransparency = S.ChamsFillTransparency
                    d.Highlight.OutlineColor = S.ChamsOutlineColor
                    d.Highlight.OutlineTransparency = S.ChamsOutlineTransparency
...
function ESP:Listen(container)
    ...
    for _, model in pairs(container:GetChildren()) do
        if model:IsA("Model") then self:Add(model) end
    end
